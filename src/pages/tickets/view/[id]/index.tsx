...
function TicketViewPage() {
  ...
  const queryParams = useMemo<{ include: { show_time: boolean; user: boolean } }>(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['show_time', 'user'],
          id,
        },
        'ticket',
      ),
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('show_time', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Show Time"
                    text={
                      <Link as={NextLink} href={`/show-times/view/${data?.show_time?.id}`}>
                        {data?.show_time?.start_time + ''}
                      </Link>
                    }
                  />
                )}
