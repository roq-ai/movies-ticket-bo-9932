...
import { useRoqClient, useShowTimeFindFirst } from 'lib/roq';
import * as RoqTypes from 'lib/roq/types';
import { convertQueryToPrismaUtil } from 'lib/utils';
import { showTimeValidationSchema } from 'validationSchema/show-times';
...
function ShowTimeEditPage() {
  ...
  const { data, error, isLoading, mutate } = useShowTimeFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ShowTimeInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.show_time.update({
        data: values as RoqTypes.show_time,
        where: {
          id,
        },
      });
  ...
  const formik = useFormik<ShowTimeInterface>({
    initialValues: data,
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <Flex justifyContent={'flex-start'}>
            ...
            <Button
              ...
              gap="0.5rem"
              mr="4"
              onClick={() => router.push('/show-times')}
              _hover={{
                bg: 'neutral.transparent',
                color: 'neutral.main',
              }}
