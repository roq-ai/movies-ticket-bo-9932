...
import { useShowTimeFindFirst, useRoqClient } from 'lib/roq';

import { ShowTimeInterface } from 'interfaces/show-time';
import { TicketListPage } from 'pages/tickets';

function ShowTimeViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const roqClient = useRoqClient();
  const queryParams = useMemo<{ include: { movie: boolean; cinema: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useShowTimeFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('show_time', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/show-times/edit/${id}`} passHref legacyBehavior>
                    <Button
